...
{if $bad || $bad_slaves}<h4>Zones need to be check or modify</h4><p>{if $bad}Master: {section name=i loop=$bad}<a class=attention title="Modify" alt="Modify" href="{$src}record.php?i={$bad[i].id}"><b>{$bad[i].name}</b></a>|<a class=attention title="Check" alt="Check" href="{$src}zonelist.php?i={$bad[i].id}&check=y"><b>&check;</b></a>&nbsp;&nbsp;&nbsp;{/section}<br />{/if}
